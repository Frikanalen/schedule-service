...
def MetadataOriginationInformationTable(main):
    MetadataOriginationInformationTable = SubElement(
        main, "tva:MetadataOriginationInformationTable"
    )
    MetadataOriginationInformation = SubElement(
        MetadataOriginationInformationTable,
        "tva:MetadataOriginationInformation",
...
def ProgramInformationTable(ProgramDescription, DailySchedule):
    ...
    videos = {
        x.video.ID: (x.video.name, x.organization.name, x.video.CRID)
        for x in DailySchedule["items"]
    }
    for video_id, (video_name, org_name, CRID) in videos.items():
        ProgramInformation = SubElement(
            ProgramInformationTable, "ProgramInformation", {"programId": CRID}
        )
...
def ServiceInformationTable(ProgramDescription):
    ...
    ServiceInformation = SubElement(
        ServiceInformationTable, "ServiceInformation", {"serviceId": "frikanalen.no"}
    )
